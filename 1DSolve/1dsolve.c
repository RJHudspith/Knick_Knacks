...
int main( void )
{
  Integrator integrator = initialise_integrator( RADAU5 ,
						 1E-5 ) ;

  const double actual = -0.75*exp(-2*0.2)+0.5*0.2+1.75 ;
  const double result = integrate( fxy , &integrator , 0 , 0.2 , 1 ) ;
 
  printf( "Finished in %zu steps\n" , integrator.nsteps ) ;
  printf( "Acceptance :: %f \n" , 
	  100*(integrator.nsteps-integrator.notoksteps)
	  /(double)integrator.nsteps ) ; 
  printf( "Result :: %g || Error :: %e \n" , result , result-actual ) ;
  return 0 ;
}
