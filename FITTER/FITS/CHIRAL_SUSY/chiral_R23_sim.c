...
void
chiral_R23_sim_guess( double *__restrict params ,
		       void *data ,
		       const int NPARAMS )
{
  ...
  }
  {
    ...
    double *coeffs2 = compute_guess( DATA , xposit , 1 ) ;
    xposit += ( DATA->NDATA[1] ) ;
    params[5] = ( coeffs1[0] + coeffs2[0] ) / 2.0 ;
    params[6] = -1.0 ;
    free( coeffs1 ) ; free( coeffs2 ) ;
  }
  {
    ...
    double *coeffs2 = compute_guess( DATA , xposit , 1 ) ;
    xposit += ( DATA->NDATA[1] ) ;
    params[7] = ( coeffs1[0] + coeffs2[0] ) / 2.0 ;
    params[8] = -1.0 ;
    free( coeffs1 ) ; free( coeffs2 ) ;
  }
...
double
chiral_R23_sim_eval( const double *__restrict x ,
		      const struct x_descriptor X ,
		      const int NPARAMS )
{
  return x[0] * ( 1.0 + 
...
int
chiral_R23_sim_f( const gsl_vector *__restrict x , 
	void *data , 
  ...
  for( NSIM = 0 ; NSIM < DATA->SIMS ; NSIM++ ) { // loop simultaneous datasets
    ...
    for( i = lopos ; i <= hipos ; i++ ) {
      ...
      gsl_vector_set ( f , count , 
		       ( chiral_R23_sim_eval( p , XAXIS , DATA->NPARAMS ) - DATA->y[i] ) / DATA->sigma[i] ) ;
      count++ ;
    }
...
int
chiral_R23_sim_df ( const gsl_vector *__restrict x, 
		     void *data, 
		     gsl_matrix *__restrict J )
{
  ...
  for( NSIM = 0 ; NSIM < DATA-> SIMS ; NSIM++ ) {
	...
	gsl_matrix_set( J , count , 5 , _s * ( 1 
					       + DATA->X[i] * ( params[ 1 ] + fac * ( log( DATA->X[i] / ( chiral * chiral ) ) / ( 16 * M_PI * M_PI ) ) ) 
					       + params[6] / ( DATA->quarks[i].ainverse * DATA->quarks[i].ainverse )
					       ) ) ;
	gsl_matrix_set( J , count , 1 , _s * DATA->X[i] * params[5] ) ;
	gsl_matrix_set( J , count , 6 , _s * params[5] / ( DATA->quarks[i].ainverse * DATA->quarks[i].ainverse )  ) ;
      // fit R4
      } else if( NSIM/2 == 3 ) {
	gsl_matrix_set( J , count , 7 , _s * ( 1 
					       + DATA->X[i] * ( params[ 1 ] + fac * ( log( DATA->X[i] / ( chiral * chiral ) ) / ( 16 * M_PI * M_PI ) ) ) 
					       + params[8] / ( DATA->quarks[i].ainverse * DATA->quarks[i].ainverse )
					       ) ) ;
	gsl_matrix_set( J , count , 1 , _s * DATA->X[i] * params[7] ) ;
	gsl_matrix_set( J , count , 8 , _s * params[7] / ( DATA->quarks[i].ainverse * DATA->quarks[i].ainverse )  ) ;
      }
...
int
chiral_R23_sim_fdf (const gsl_vector *__restrict x , 
		     void *data ,
		     gsl_vector *__restrict f , 
		     gsl_matrix *__restrict J )
{
  chiral_R23_sim_f( x , data , f ) ;
  chiral_R23_sim_df( x , data , J ) ;
  return GSL_SUCCESS;
}
