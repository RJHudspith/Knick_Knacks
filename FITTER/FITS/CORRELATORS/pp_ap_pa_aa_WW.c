	       ...
	       const int NPARAMS )
{
  ...
  for( k = 0 ; k < 1 ; k++ ) {
    ...
    PPAA_WW_description( "PPAA_WW guess" , p , X , 2 ) ;
  }
	       ...
	       const size_t chan )
{
  ...
  case PWAL : // AWPL
    ...
    // WALL-WALL
  case PWPW :
    return x[3] * x[3] * ( exp( -x[0] * X.X ) + exp( -x[0] * ( X.LT - X.X ) ) ) ;
  case AWAW :
    return x[4] * x[4] * ( exp( -x[0] * X.X ) + exp( -x[0] * ( X.LT - X.X ) ) ) ;
  case PWAW :
    return x[3] * x[4] * ( exp( -x[0] * X.X ) - exp( -x[0] * ( X.LT - X.X ) ) ) ;
  case AWPW :
    return x[3] * x[4] * ( exp( -x[0] * X.X ) - exp( -x[0] * ( X.LT - X.X ) ) ) ;
  }
  return 0 ;
  ...
  for( NSIM = 0 ; NSIM < DATA->SIMS ; NSIM++ ) {
    ...
    for( i = lopos ; i <= hipos ; i++ ) {
      ...
      gsl_vector_set ( f , count , 
		       ( PPAA_WW_eval( params , XAXIS , NSIM ) - DATA->y[i] ) / DATA->sigma[i] ) ;
      count++ ;
    }
	 ...
	 gsl_matrix *__restrict J )
{
  ...
  for( NSIM = 0 ; NSIM < DATA-> SIMS ; NSIM++ ) {
	...
	//
      case PWPW :
	gsl_matrix_set ( J , count , 0 , -params[3] * params[3] * ( fw * e1 + bw * e2 ) ) ;
	gsl_matrix_set ( J , count , 3 ,  2 * params[3] * ( e1 + e2 ) ) ;
	break ;
      case AWAW :
	...
	break ;
      case PWAW :
      case AWPW :
	gsl_matrix_set ( J , count , 0 , -params[3] * params[4] * ( fw * e1 - bw * e2 ) ) ;
