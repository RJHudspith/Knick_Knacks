	     ...
	     const bool renormalise )
{
  ...
  if( NSLICES != 4 ) {
    printf( "Expected V (ll) A(ll) V(ls) A(ls)\n" ) ;
    return ;
  }
  ...
  char str[ 256 ] ;
  sprintf( str , "m%g_m%g.dat" , INPARAMS -> quarks[2].ml , INPARAMS -> quarks[2].ms ) ;
  ...
  fprintf( file , "Combination :: ll(%g %g) - ls(%g %g) (V+A;0+1)\n" , 
	   INPARAMS -> quarks[0].ml , INPARAMS -> quarks[1].ml , 
	   INPARAMS -> quarks[2].ml , INPARAMS -> quarks[3].ms ) ;
