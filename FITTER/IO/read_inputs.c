	     ...
	     const char *filename )
{
  ...
  // get the filetype default is fake data
  {
    ...
    } else if( are_equal( INPUT[analysis_idx].VALUE , "TETRA_MESONS" ) ) {
      INPARAMS -> type = TETRA_MESONS ;
    } else if( are_equal( INPUT[analysis_idx].VALUE , "TETRA_FULL" ) ) {
      INPARAMS -> type = TETRA_FULL ;
